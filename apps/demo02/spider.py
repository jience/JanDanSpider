...
def getImg(html):
    ...
    reg = r'<a href="(.+?)" target="_blank" class="view_img_link">'  # 正则表达式
    ...
    if not os.path.isdir(paths):  # 将每个页面的图片分别存放在不同的文件夹，这是后来改的，我发现所有图片放一个文件夹有点小问题
        os.makedirs(paths)
...
def donwloadimg(page):
    url = 'http://jandan.net/ooxx/page-' + str(page)+'#comments'  # 此处链接可以换成别的网站，这就需要分析网站结构了
    print('正在爬取 ' + url)
...
path = r'C:\Users\ling\Desktop\jiandan'
startpage = 1
endpage = 1
